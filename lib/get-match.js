...
var excludeStyles = ['transition', 'font', 'overflow', 'webkit', 'Moz', 'ms']
...
function getMatch(element, to){
  ...
  getStyleKeys().forEach(function(key){
    ...
    if (!(root in start) && !~excludeStyles.indexOf(root) && fromStyle[key] != toStyle[key]){
  ...
  if ((fromPos == 'static' || fromPos == 'relative') && (toPos == 'static' || toPos == 'relative')){
    ...
    var marginTopOffset = parsePx(toStyle['marginTop'])
    var marginLeftOffset = parsePx(toStyle['marginLeft'])

    start['position'] = end['position'] = 'absolute'
    start['top'] = element.offsetTop - marginTopOffset + 'px'
    start['left'] = element.offsetLeft - marginLeftOffset + 'px'
    ...
    end['top'] = element.offsetTop - marginTopOffset + 'px'
    end['left'] = element.offsetLeft - marginLeftOffset + 'px'
